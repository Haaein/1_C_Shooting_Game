...
* 
*/
//발견된 문제점 : 1. 플레이어의 총알과 함께 움직이는 적  2. 뭉쳐서 출력되는 적
...
struct ST_Enemy
{
	...
	int y[MAX];
	bool bActive[MAX];
} stEnemy; //영상에서는 stEnemy[i].x 식으로 사용하지만 C에서는 안되서 구조체 변수를 선언 후 stEnemy.x[i] 형식으로 사용함
...
void main() {
		...
		//아래의 함수는 영상 + 내가 직접 찾은 코드들을 합쳐서 만든 것이다.
	{
		...
		{
			if (!stEnemy.bActive[i]) {
				stEnemy.x[i] = (rand() % 30) * 2;
				stEnemy.y[i] = 1;
				stEnemy.bActive[i] = true;
		...
		for (i = 0; i < MAX; i++) { //for문을 통해서 각 배열의 원소별로 발사 모습 출력
			if (stEnemy.bActive[i]) { //에너미가 활성화 되었다면(bActive = true)
				gotoxy(stEnemy.x[i], stEnemy.y[i]); //설정된 적좌표(랜덤 좌표)
				printf("☆"); //총알을 출력하고
				stEnemy.y[i]++; //y좌표 증가(아래로 내려오는 효과)
				if (stEnemy.y[i] >= y - 1) //적의 위치가 플레이어의 y좌표 -1 일때
				{
					stEnemy.bActive[i] = false;
